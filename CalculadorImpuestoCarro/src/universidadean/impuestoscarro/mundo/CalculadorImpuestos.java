...
public class CalculadorImpuestos {
    ...
    private RangoImpuesto buscarRangoImpuesto(double valor) {
        ...
        rango=new RangoImpuesto(0,30000000,0.015);
        if(rango.contieneA(valor)){
            return rango;
        }
        rango=new RangoImpuesto(30000000,70000000,0.02);
        if(rango.contieneA(valor)){
            return rango;
        }
        rango=new RangoImpuesto(70000000,200000000,0.025);
        if(rango.contieneA(valor)){
            return rango;
        }
        else {

            rango=new RangoImpuesto(200000000,1000000000,0.04);
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        pago=precio*(rango.darPorcentaje());
        if(descProntoPago){
            pago=pago*0.9;
        }
    ...
    public Vehiculo buscarVehiculoMasCaro() {
        ...
        // Se crea el objeto masCaro tipo vehiculo,
        return masCaro;
    }
    ...
    public Vehiculo buscarVehiculoPorMarca(String marca) {
        ...
        for(int i=0;i<vehiculos.length;i++){
            if(marca.equalsIgnoreCase(vehiculos[i].darMarca())){
                buscado=vehiculos[i];
                break;
            }
        }
    ...
    public Vehiculo buscarVehiculoPorLinea(String linea) {
        ...
        for(int i=0;i<vehiculos.length;i++){
            if(linea.equalsIgnoreCase(vehiculos[i].darLinea())){
                buscado=vehiculos[i];
                break;
            }
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
        ...
        for(int i=1;i<vehiculos.length;i++){
            if(Long.parseLong(vehiculos[i].darAnio())< Long.parseLong(buscado.darAnio())){
                buscado=vehiculos[i];
            }
    ...
    public double promedioPreciosVehiculos() {
        ...
        for(int i=1;i<vehiculos.length;i++){
            promedio+=vehiculos[i].darPrecio();
